...
void* ft_coders(void* arg)
{
    ...
    while(1)
    {
        ...
        {
            if ((min_time == -1 )|| (min_time > data->cnx_array[i].last_com_time + data->time_to_burnout))
            {
                min_time = data->cnx_array[i].last_com_time + data->time_to_burnout;
                id_time = data->cnx_array[i].id;
            }
            i += 1;
        }
        while((usb[left % num] != 1) || (usb[right % num] != 1) || id != id_time)
        {
